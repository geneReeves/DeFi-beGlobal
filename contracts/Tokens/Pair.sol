...
contract Pair is IPair, PancakeERC20 {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "GlobalPair: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "GlobalPair: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "GlobalPair: INSUFFICIENT_LIQUIDITY"
        );
        ...
        require(
            ...
            "GlobalPair: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
            // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint256 _swapFee = swapFee;
            uint256 balance0Adjusted = (
                balance0.mul(100000).sub(amount0In.mul(_swapFee))
            );
            uint256 balance1Adjusted = (
                balance1.mul(100000).sub(amount1In.mul(_swapFee))
            );
            require(
                balance0Adjusted.mul(balance1Adjusted) >=
                    uint256(_reserve0).mul(_reserve1).mul(100000**2),
                "GlobalPair: K"
            );
        }
        //Original SwapFee base
