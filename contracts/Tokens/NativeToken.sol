...
contract NativeToken is BEP20 {
    ...
    event MaxTransferAntiWhale(
        address indexed devPower,
        uint256 oldAntiWhalePercent,
        uint256 newAntiWhalePercent
    );
    ...
    function GetIfExcludedFromAntiWhale(address addr)
        public
        view
        returns (bool)
    {
    ...
    function setExcludedFromAntiWhale(address addr, bool _excluded)
        public
        onlyDevPower
    {
    ...
    function maxTokensTransferAmountAntiWhaleMethod()
        public
        view
        returns (uint256)
    {
    ...
    function updateMaxTransferAntiWhale(uint16 _newAntiWhalePercent)
        public
        onlyDevPower
    {
        require(
            _newAntiWhalePercent <= MAX_ANTIWHALE,
            "[!] Antiwhale method triggered. You are trying to set a % which is too high Check MAX_ANTIWHALE in the SC."
        );
        emit MaxTransferAntiWhale(
            msg.sender,
            antiWhalePercent,
            _newAntiWhalePercent
        );
    ...
    modifier antiWhale(
        address origen,
        address destinataria,
        uint256 q
    ) {
        if (maxTokensTransferAmountAntiWhaleMethod() > 0) {
            if (
                _antiWhaleWhiteList[origen] == false &&
                _antiWhaleWhiteList[destinataria] == false
            ) {
                require(
                    q <= maxTokensTransferAmountAntiWhaleMethod(),
                    "[!] Antiwhale method triggered. You are trying to transfer too many tokens. Calm down and don't panic sell bro."
                );
            }
        }
        _;
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "GLOBAL::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "GLOBAL::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "GLOBAL::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "GLOBAL::_writeCheckpoint: block number exceeds 32 bits"
        );
