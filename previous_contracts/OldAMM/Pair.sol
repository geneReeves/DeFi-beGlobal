...
contract Pair is IPair, PancakeERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1)
        ...
        returns (bool feeOn)
    {
        (address devFeeTo, uint256 devFeeNum, uint256 devFeeDenum) = IFactory(
            factory
        ).getDevFee();
            ...
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint256 denominator = rootK
                        .mul(devFeeDenum.sub(devFeeNum))
                        .add(rootKLast.mul(devFeeNum));
    ...
    function swap(
        ...
        );
        {
            ...
            uint256 swapFeeAm = IFactory(factory).getSwapFee();
            uint256 balance0Adjusted = (
                balance0.mul(10000).sub(amount0In.mul(swapFeeAm))
            );
            uint256 balance1Adjusted = (
                balance1.mul(10000).sub(amount1In.mul(swapFeeAm))
            );
            require(
