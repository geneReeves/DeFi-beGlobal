...
contract SmartChef is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        require(
            ...
            "Start block must be before than bonus end block"
        );
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare.add(
                globalReward.mul(PRECISION_FACTOR).div(stakedTokenSupply)
            );
            return
                user
                    .amount
        ...
        returns (uint256)
    {
        ...
        if (_from <= startBlock && _to >= bonusEndBlock)
            return bonusEndBlock.sub(startBlock);
        if (_from > startBlock && _from < bonusEndBlock && _to >= bonusEndBlock)
            return bonusEndBlock.sub(_from);
        if (_to > startBlock && _to < bonusEndBlock && _from <= startBlock)
