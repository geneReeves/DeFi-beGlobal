    ...
    function buyTokens(uint256 quantity, address buyer) private onlyHuman {
        require(
            (getStatus() == 0 && whitelist[buyer] && bnbacc < hardcap) ||
                ...
                (getStatus() == 1 &&
                    publicBegins.add(1 hours) > block.timestamp),
            "[ERROR: YOU ARE NOT ALLOWED TO BUY]"
        );
        require(
            quantityBought[buyer].add(quantity) <= 25e18,
            "[ERROR: THANK YOU FOR YOUR KIND SUPPORT, BUT YOU CAN NOT BUY SO MANY TOKENS. Wait for launch to purchase more]"
        );
        ...
        if (getStatus() == 0 && whitelist[buyer] && bnbacc < hardcap) {
            ...
            getStatus() == 1 && publicBegins.add(1 hours) > block.timestamp
        ) {
